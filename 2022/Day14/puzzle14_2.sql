...
select o.y,
	...
	where i.y = o.y and i.x between 300 and 600
	for xml path(''),type).value(N'.[1]', N'nvarchar(max)'), 1, 2, N'') row
from cave o
where o.y between 0 and 180
group by o.y
order by o.y
...
while exists(select * from cave where x = 500 and y = 0 and point = '.') and @resting=1
begin
	...
	--select @resting resting, @sandy + 1
end


select o.y,
	STUFF((select ' ' + point from cave i
	where i.y = o.y and i.x between 0 and 1000
	for xml path(''),type).value(N'.[1]', N'nvarchar(max)'), 1, 2, N'') row
from cave o
where o.y between 0 and 355
group by o.y
order by o.y

select count(*) part2 from cave where point = 'o'
